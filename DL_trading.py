...
# Hyper Parameters
EPOCH = 80      # train the training data n times, to save time, we just train 1 epoch
BATCH_SIZE = 1024
...
def model_train(loader_train,loader_test):
    ...
    optimizer = torch.optim.Adam(msresnet.parameters(), lr=0.005,weight_decay= 0.07)
    ...
    for epoch in range(EPOCH):
            ...
            if train_accuracy >= 90 and profit > big_profit :
                big_profit = profit
                torch.save(msresnet,"2015-2016NewNew_wegiht_decay007.pkl")
...
def draw(train_loss, train_acc,winrate):
    ...
    df = pd.DataFrame({'loss': train_loss,
                   'acc': train_acc,
                   'winrate': winrate})
    df.to_csv("2015-2016NewNew_wegiht_decay007.csv",index=False)
