...
path_to_choose2013 = "./gt_25action_0121(Method3)/2013/"
            ...
            for pair in range(len(table)):
                    ...
                    if Use_avg :      
                        ...
                        if normalize_spread == True :
                            spread[:] = (spread[:] - table.mu[pair])/table.Estd[pair]
                            new_spread = np.zeros((1,512))
...
def test_data():
    ...
    for date in sorted(datelist[:]): 
            ...
            if Use_avg : avgmin = pd.read_csv(test_period[time][1]+date+ext_of_average)
            else : halfmin = pd.read_csv(test_period[time][2]+date+ext_of_half)
