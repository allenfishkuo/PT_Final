# -*- coding: utf-8 -*-
"""
Created on Wed Jan 20 15:25:31 2021
...
trading_cost_threshold = np.arange(0.0015,0.008,0.0005)
number_of_label = 2
#train_data, train_label, test_data, test_label = dataloader.read_data()
def DataTransfer(train_data, train_label):
...
class MSResNet(nn.Module):
    def __init__(self, input_channel, layers=[1, 1, 1, 1], num_classes=number_of_label):
        self.inplanes3 = 64
        self.inplanes5 = 64
        self.inplanes7 = 64
...
def model_train(loader_train):
    ...
    """
    msresnet = MSResNet(input_channel=3, layers=[1, 1, 1, 1], num_classes=number_of_label) #channel 設定
    msresnet = msresnet.cuda()
    criterion = nn.CrossEntropyLoss(size_average=False).cuda()
    optimizer = torch.optim.Adam(msresnet.parameters(), lr=0.01,weight_decay=0.005)
    #optimizer = torch.optim.SGD(msresnet.parameters(), lr=0.01,momentum = 0.9)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[10,20,30,40,50], gamma=0.3	)
...
if __name__=='__main__':
    choose = 0
    if choose == 0 :
        train_data, train_label = new_dataloader.val_data()
        loader_train = DataTransfer(train_data, train_label)
        model_train(loader_train)
    else :
        test.test_reward()
